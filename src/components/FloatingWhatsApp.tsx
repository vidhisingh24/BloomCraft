...
export const FloatingWhatsApp: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 flex items-center gap-2 sm:gap-3">
      ...
      {showTooltip && (
        <div className="hidden md:flex items-center gap-2 px-3.5 py-2 bg-white/95 backdrop-blur-md rounded-full shadow-lg border border-rose-200/80 text-xs text-[#3D272A] animate-float-slow">
          <Sparkles className="w-3.5 h-3.5 text-[#D96B82]" />
          <span>Chat with artisan & order directly!</span>
          <button
            onClick={() => setShowTooltip(false)}
            className="text-[#A4838B] hover:text-[#3D272A] ml-1 p-0.5 cursor-pointer"
      ...
      >
        <MessageCircle className="w-4 h-4 sm:w-5 sm:h-5 fill-white shrink-0 group-hover:rotate-12 transition-transform" />
